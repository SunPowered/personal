...
class SpiceRead(object):
    ...
    def readFile(self,filename):
        ...
        while (1):
            ...
            if keyword == "title":
                self.current_plot.setAttributes(title=tok[1])
            elif keyword == "date":
                self.current_plot.setAttributes(date=tok[1])
            elif keyword == "plotname":  ## FIXME: incomplete??
                self.current_plot.setAttributes(plotname=tok[1])
            elif keyword == "flags":
                ...
                for i in xrange(self.nvars):
                    ...
                    if len(line) >= 3:
                        ...
                        curr_vector = SpiceVector(name=line[1],
                                                   type=line[2])
                ...
                if self.real:
                    ...
                    self.vectors[0].setData(aa[:,0])
                    self.current_plot.setScaleVector(self.vectors[0])
                    for n in xrange(1,self.nvars):
                        self.vectors[n].setData(aa[:,n])
                        self.current_plot.appendDataVector(self.vectors[n])
                    ...
                    self.vectors[0].setData(aa[:,0]) ## only the real part!
                    self.current_plot.setScaleVector(self.vectors[0])
                    for n in xrange(1,self.nvars):
                        self.vectors[n].setData(numpy.array(aa[:,2*n]+
                                                             1j*aa[:,2*n+1]))
                        self.current_plot.appendDataVector(self.vectors[n])
...
if __name__ == "__main__":
    ...
    for f in sys.argv[1:]:
        ...
        for i,p in enumerate(SpiceRead(f).getPlots()):
            ...
            for j,d in enumerate(p.getDataVectors()):
