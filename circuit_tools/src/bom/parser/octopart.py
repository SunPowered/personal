...
import urllib2
import json
from types import IntType, ListType, LongType, StringType, NoneType, BooleanType, DictType, FloatType, TupleType
...
class OctopartBrand(object):
	...
	def __init__(self, _id, dispname, homepage):
		self._id = _id
...
class Octopart(object):
	...
	def categories_get(self, _id):
		...
		args = self._categories_get_args(_id)
		try:
	...
	def _categories_get_multi_args(self, ids):
		...
		self._validate_args(args, arg_types, arg_ranges)
		for _id in args['ids']:
			if type(_id) not in (IntType, LongType):
				raise OctopartException(args, arg_types, arg_ranges, 2)
	...
	def _parts_search_args(self, args):
		...
		for _filter in args.get('filters', []):
			if len(_filter) != 2:
				raise OctopartException(args, arg_types, arg_ranges, 9)
			if isinstance(_filter[0], basestring) is False:
				raise OctopartException(args, arg_types, arg_ranges, 2)
			if type(_filter[1]) is not ListType:
				raise OctopartException(args, arg_types, arg_ranges, 2)
		
		for _filter in args.get('rangedfilters', []):
			if len(_filter) != 2:
				raise OctopartException(args, arg_types, arg_ranges, 9)
			if isinstance(_filter[0], basestring) is False:
				raise OctopartException(args, arg_types, arg_ranges, 2)
